...
function calculateCommitQuality(commitMessages: string[]): number {
  ...
  commitMessages.forEach((msg) => {
    ...
    if (
      msg.match(
        /\b(fix|refactor|optimize|improve|feature|add|remove|resolve|implement)\b/i
      )
    )
      score += 2
    ...
    if (
      msg.match(
        /\b(api|database|security|performance|memory|function|bug|patch|error|deployment)\b/i
      )
    )
      score += 2
    ...
    if (
      msg.includes("#") ||
      msg.toLowerCase().includes("fixes") ||
      msg.toLowerCase().includes("closes")
    ) {
      score += 3
    }

    // ❌ Penalizing Bad Commit Messages
    if (msg.match(/\b(update|more work|changes|edit|modified|updated|wip)\b/i))
      score -= 2
...
export const addCandidate = async (
  ...
  try {
    ...
    if (candidate) {
      ...
      // Find or update the match percentage for this company
      ...
      if (existingMatchIndex !== -1) {
        // Update existing match percent entry
        candidate.matchPercentByCompany[existingMatchIndex].matchPercent =
          matchPercent
      } else {
        // Add new entry
    ...
    const newCandidate = new Candidate({
      ...
      matchPercentByCompany: [{ companyId, matchPercent }], // Store per-company match percent
    })
...
export const getCandidatesForCompany = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
